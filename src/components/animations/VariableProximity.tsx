...
const VariableProximity = forwardRef<HTMLSpanElement, VariableProximityProps>((props, ref) => {
  ...
  const getFontWeight = (falloffValue: number) => {
    // For Korean text, use font-weight instead of font-variation-settings
    const minWeight = 400;
    const maxWeight = 900;
    return Math.round(minWeight + (maxWeight - minWeight) * falloffValue);
  };
  ...
  useAnimationFrame(() => {
    ...
    letterRefs.current.forEach((letterRef, index) => {
      ...
      const hasKorean = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/.test(label);
      
      if (hasKorean) {
        // Use font-weight for Korean text
        const fontWeight = getFontWeight(falloffValue);
        letterRef.style.fontWeight = fontWeight.toString();
        letterRef.style.fontVariationSettings = "";
      } else {
        // Use font-variation-settings for English text
